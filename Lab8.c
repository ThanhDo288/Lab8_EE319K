...
volatile  uint32_t Data;        // 12-bit ADC
...
uint32_t Convert(uint32_t x){
  float max_distance=5.0;//5cm
	float max_ADC_value=4095.0; //12bit
	float resolution=max_distance/max_ADC_value;
  // distance = (uint32_t)(x * resolution); // 0.001 cm = 1000 µm
	int16_t distance_fixed_point = (int16_t)((x * resolution)*1000);
	return distance_fixed_point;
...
int main(void) { 
  ...
  uint32_t out1;
	...
	uint32_t average=0;
  EnableInterrupts();
// Initialize the LCD plot
  ST7735_PlotClear(0, 4095); // Clear the plot area
 // ST7735_PlotTitle("ADC Data vs. Time", "Time (s)", "ADC Value");
  
    while (1) {
      if (ADCstatus == 1) {
        average = ADCmail;
        ADCstatus = 0;
        out2 = Convert(average);
        debug = Convert(ADCmail);
        ST7735_SetCursor(0, 0);
        ST7735_OutString("ADC: ");
        ST7735_OutUDec(average); // print
				ST7735_OutString("\n"); //  cm
				ST7735_OutFix(out2); // print
        ST7735_OutString("cm "); //  
				
				// Plot the ADC value
        ST7735_PlotPoint(average);
